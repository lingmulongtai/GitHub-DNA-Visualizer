 ...
 */
function radarPolygonPoints(cx, cy, radius, values) {
    return values
        .map((v, i) => {
 ...
 */
async function generateCardData(data) {
    const W = 1200;
    const H = 630;
    ...
    const radarGridLines = [33, 66, 100]
        .map((pct) => `<polygon points="${pentagonPoints(radarCX, radarCY, (radarR * pct) / 100)}" fill="none" stroke="${BORDER}" stroke-width="1"/>`)
        .join('\n    ');
